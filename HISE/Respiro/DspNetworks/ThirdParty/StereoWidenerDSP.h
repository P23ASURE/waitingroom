...
#include "src/StereoWidenerDSP.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct StereoWidenerDSPMetaData {
		SN_NODE_ID("StereoWidenerDSP");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using StereoWidenerDSP = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _StereoWidenerDSP, StereoWidenerDSPMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
